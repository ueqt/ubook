...
import { changePath, changeOreillyPath } from "../../../utils/syncUtils/common";
...
> {
  ...
  syncFromOreillyLocation = async () => {
    ...
    let sourcePath = path.join(oreillyLocation, "_index");
    ...
    if (result) {
      ...
      : window
          .require("electron")
          .ipcRenderer.sendSync("storage-location", "ping");
      if(!fs.existsSync(path.join(storageLocation, 'oreilly'))) {
        fs.mkdirSync(path.join(storageLocation, 'oreilly'));
      }
      fs.writeFileSync(path.join(storageLocation, 'oreilly', 'all.json'), JSON.stringify(result), {encoding: 'utf-8'});
      toast.success(this.props.t("Change Successfully"));
  ...
  handleChangeOreillyLocation = async () => {
    const { ipcRenderer } = window.require("electron");
    const path = await ipcRenderer.invoke("change-path");
    if (!path.filePaths[0]) {
      return;
    }
    let result = await changeOreillyPath(
      localStorage.getItem("oreillyLocation")
        ? localStorage.getItem("oreillyLocation")
        : ipcRenderer.sendSync("oreilly-location", "ping"),
      path.filePaths[0]
    ...
    localStorage.setItem("oreillyLocation", path.filePaths[0]);
    document.getElementsByClassName(
      "setting-dialog-oreilly-location-title"
    )[0].innerHTML =
      path.filePaths[0] ||
      localStorage.getItem("oreillyLocation") ||
      ipcRenderer.sendSync("oreilly-location", "ping");
          ...
          <div className="setting-dialog-new-title">
            ...
            <ul className="theme-setting-container">
              {themeList.map((item, index) => (
                <Tippy
                  key={item.id}
                  content={this.props.t(item.title)}
                >
                  <li
                    className={
            ...
            <>
              <div className="setting-dialog-new-title">
                ...
                <span
                  className="change-location-button"
                  onClick={() => {
                    this.handleChangeOreillyLocation();
                  }}
                >
                  <Trans>Change location</Trans>
                </span>
              </div>
              <div className="setting-dialog-oreilly-location-title">
                {localStorage.getItem("oreillyLocation")
                  ? localStorage.getItem("oreillyLocation")
                  : window
                      .require("electron")
